...
for epoch in range(num_epochs):
    train_mae = []
    for i,(data, _, label) in enumerate(train_loader):
        label = label.float()
        #print(label)
        ...
        if (i+1) % 100 == 0:
            ...
            torch.save(model.state_dict(), 'deep_learning/dbp/saved/modelb%d.ckpt'%epoch)
            if loss<min_loss:
                min_loss = loss
                torch.save(model.state_dict(), 'deep_learning/dbp/modelb.ckpt')
            output_list = list()
            label_list = list()    
            for i,(data,_,label) in enumerate(test_loader):
                label = label.float()
                #print(data,label)
    ...
    with open(r'./deep_learning/dbp/results.csv', 'a', newline='') as csvfile:
